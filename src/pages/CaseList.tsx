...
const CaseList: React.FC = () => {
    ...
    return (
        <div className="w-full h-screen mx-auto p-12">
                ...
                <div className="flex flex-row gap-4">
                    <div className="flex-1 relative">
                        <input
                            type="text"
                            placeholder="Search.."
                            className="w-64 px-4 py-2 text-sm border border-gray-dark rounded-md focus:outline-none focus:border-gray-dark"
                        />
                    </div>
                    <button 
                        onClick={() => setShowModal(true)}
                        className="px-4 py-2 text-sm bg-gray text-black font-500 rounded-md border border-gray-dark hover:bg-gray-light transition-colors duration-150"
                    >
                        New Case
                    </button>
                </div>
            ...
            <div className="bg-white rounded-lg p-4">
                <table className="w-full table-auto text-left">
                    <thead className="bg-gray">
                        <tr>
                            <th className="p-3 font-500 text-sm align-middle">Case Name</th>
                            <th className="p-3 font-500 text-sm align-middle">Client</th>
                            <th className="p-3 font-500 text-sm align-middle">Dates</th>
                            <th className="p-3 font-500 text-sm align-middle">Attachments</th>
                            <th className="p-3 font-500 text-sm align-middle">Teammates</th>
                        </tr>
                    </thead>
                    <tbody>
                        {tasks.map((item, index) => (
                            <tr key={index} className="border-b border-gray hover:bg-gray-light">
                                <Link to={`/case/${item.id}`} className="contents">
                                    <td className="p-3 align-middle">{item.name}</td>
                                    <td className="p-3 align-middle">{item.client}</td>
                                    <td className="p-3 align-middle">{item.dates}</td>
